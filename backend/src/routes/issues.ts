...
router.put('/issues/:id', (req, res) => {
  try {
    ...
    const photos = photo_filenames !== undefined ? JSON.stringify(photo_filenames) : existing.photo_filenames;
    db.prepare(
      ...
      tracking_status ?? existing.tracking_status, comment ?? existing.comment,
      photos, severity ?? existing.severity, req.params.id
    );
